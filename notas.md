...
La metrica principal para comparar modelos fue PR-AUC porque el objetivo es ordenar correctamente a los clientes con mayor riesgo dentro de un problema desbalanceado. Se probaron 10 alternativas: Dummy, regresion logistica, arbol de decision, Random Forest, Gradient Boosting, Extra Trees, AdaBoost, Gaussian Naive Bayes, LightGBM y CatBoost, todos con validacion cruzada y busqueda de hiperparametros acotada. Bajo este criterio, el modelo final seleccionado fue CatBoost. Luego se optimizo el umbral con F2-score y se obtuvo un umbral de 0.31. Esta configuracion logra un recall alto, detectando la mayor parte de los clientes que efectivamente abandonan, aunque acepta una mayor cantidad de falsos positivos.
