"""Entrena y guarda exclusivamente el modelo final de churn.
...
def select_threshold(pipeline: Pipeline, X_train: pd.DataFrame, y_train: pd.Series) -> float:
    ...
    for threshold in np.round(np.arange(0.10, 0.91, 0.01), 2):
        ...
        candidates.append((
            fbeta_score(y_val, predictions, beta=2, zero_division=0),
...
def main() -> None:
    args = parse_args()
    data = build_features(pd.read_csv(args.data))
    ...
    threshold = select_threshold(pipeline, X_train, y_train)
    ...
    print(f"Umbral F2: {threshold:.2f}")
